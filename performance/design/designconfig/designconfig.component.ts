...
@Component({
  selector: 'ngx-designconfig',
  styleUrls: ['./designconfig.component.scss'],
  templateUrl: './designconfig.component.html',
})

export class DesignConfigComponent {
...
//UIP=kPb1TguiqNNfK28stVab7GONh0MVC4PSlvXeN7D5r67.0LDXa8GWH3&USR=201503040&PASS=BHFIB+8VcF5urJs/syTbYA==
  ...
  periodid:any;
	...
	MODELID:any;
	 ...
	 WSElementId:any;
	 drpElementVal:any;
	 drpFilterTypeVal:any;
	 drpShowRefferedVal:any;
	 drpScorecardsVal:any;
	 drpElementTypesVal:any;
	 drpElementTypesValArr:any = [];
	 drpDrillDownLevel:any;
	 qry:any;
	 selText:any;
	 selValue:any;
	...
	getQPRQuery()
	{
		this.selText = $("#drpSeries option:selected").text();
		this.selValue = $("#drpSeries").val();
		this.qry = "HI:1:0:1";
		this.qry = this.qry + "%2cPV:4"+this.selText+":0:1:"+this.selValue+".d.0";
		return this.qry;
	}
	...
	loadTableData(){  
		 ...
		 this.drpElementVal = $("#drpElements").val();
		 //alert(this.drpElementVal);
		 this.drpElementVal = this.drpElementVal.replace("SC.","1_0_").replace(".","_");
		 this.drpDrillDownLevel = $("#drpExpandLevel").val();
		 this.drpFilterTypeVal = $("#drpFilterType").val();
		 this.drpShowRefferedVal = $("#drpShowReffered").val();
		 this.drpScorecardsVal = $("#drpScorecards").val();
		 this.drpScorecardsVal = this.drpScorecardsVal.replace("SC.","1_0_").replace(".","_");
		 this.drpElementTypesVal = $("#drpElementTypes  option:selected").text();
		 this.drpElementTypesValArr.push(this.drpElementTypesVal);
		 console.log(this.drpElementTypesValArr);
		 console.log(this.drpScorecardsVal);
		...
		var tableData = this.getFilteredTableData(this.drpElementVal,this.getQPRQuery(),this.drpDrillDownLevel,this.drpFilterTypeVal ,this.drpShowRefferedVal,this.drpScorecardsVal,this.drpElementTypesValArr,this.periodid);		 
		 ...
		 $.each($("#tblSeriesColumns > tbody > tr > td > div > input"), function(i, item) {
			$("#tblNavigator"+' thead tr').append((<HTMLInputElement>this).value);
			console.log((<HTMLInputElement>this).value);
		 });
...
getFilteredTableData(elementID, dataquery, expandlevel, filtertype, showreffered, scorecardId, selectedelementtypes, periodID){
	...
	return this.ajaxCall("LoadElementsData",parameters);
     ...
     getTable() {
        var uname = "201503040"; var upass = "BHFIB+8VcF5urJs/syTbYA=="; var query = "[SC.274840175].Series"; var criteria = ""; var sortby = ""; var attributes = "id,name,typename,iconurl,level,scorecardid"; var options = "";
